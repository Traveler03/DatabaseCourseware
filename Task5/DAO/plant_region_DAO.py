...
class plant_region_dao_Impl(base_dao, plant_region_dao):
    ...
    def insert(self, plant_region):
        ...
        cursor.execute("insert into plant_region (plant_id, region_id) values(%s,%s)",(plant_region._plant_id,plant_region._region))
        self.connect.commit()
        cursor.close()

    def update(self, plant_region):
        cursor = self.connect.cursor()
        cursor.execute("UPDATE plant_region SET region_id=%s WHERE plant_id=%s", (
    ...
    def selectall(self):
        cursor = self.connect.cursor()
        cursor.execute("select * from plant_region")
        result = cursor.fetchall()
        cursor.close()
        return result