...
class plant_region_dao_Impl(base_dao,plant_region_dao):
    ...
    def insert(self, plant_region):
        ...
        cursor.execute("insert into plant_region values(%s,%s)",(plant_region._plant_id, plant_region._region_id))
        self.connect.commit()
        cursor.close()
    def delete(self, plant_region):
        cursor = self.connect.cursor()
        cursor.execute("delete from plant_region where plant_id=%s and class_id=%s", (plant_region._plant_id,plant_region._region_id))
        self.connect.commit()
        cursor.close()
    def select(self,sql):
        cursor = self.connect.cursor()
