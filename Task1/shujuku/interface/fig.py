...
def display_data(table, page_number):
    ...
    # 在界面中创建表格
    table['columns'] = ("PlantName", "Image", "Location", "Photographer", "Description") # 列名
    ...
    table.column("Location", anchor=tk.CENTER, width=200)
    table.column("Photographer", anchor=tk.CENTER, width=200)
    table.column("Description", anchor=tk.CENTER, width=150)

    table.heading("#0", text='', anchor=tk.CENTER)
    table.heading("PlantName", text="植物名", anchor=tk.CENTER)
    table.heading("Image", text="照片", anchor=tk.CENTER)
    table.heading("Location", text="拍摄位置", anchor=tk.CENTER)
    table.heading("Photographer", text="拍摄人员", anchor=tk.CENTER)
    table.heading("Description", text="照片描述", anchor=tk.CENTER)
...
def plant_main_window(root,user):
    ...
    def pig_insert_window():
        create_plant_entry_gui(table,user)
    # 创建按钮
    button_add = ttk.Button(table, text="增加", command=pig_insert_window)
    button_add.pack(pady=10)
    table.mainloop()
root=Tk()
plant_main_window(root,None)