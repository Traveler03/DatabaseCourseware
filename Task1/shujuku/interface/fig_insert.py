...
def create_plant_entry_gui(root,users):
    def save_entry():
        ...
        sql=("select fig_id from fig where fig_path=%s")
        fig_id_result=figs.select(sql,dest_folder)
        ...
        new_plant_fig.__int__(plant_id=plant_id,image_id=fig_id_result[0][0])
        print(plant_id,fig_id_result)
    ...
    name_label.pack()
    # 将查询结果放在植物名的下拉列表框中
    name_entry = ttk.Combobox(window,textvariable=selected_province,  values=name_values)
    name_entry.current(0)
    name_entry.pack()
